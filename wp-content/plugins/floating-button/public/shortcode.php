...
if ( count( $result ) > 0 ) {
	foreach ( $result as $key => $val ) {
		...
		if ( empty( $param['disable_fontawesome'] ) ) {
			$url_icons = $this->plugin['url'] . 'assets/vendors/fontawesome/css/fontawesome-all' . $pre_suffix . '.css';
			wp_enqueue_style( $slug . '-fontawesome', $url_icons, null, '5.14' );
		}

		$url_style = $this->plugin['url'] . 'assets/css/style' . $pre_suffix . '.css';
